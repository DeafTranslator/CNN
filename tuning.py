...
new_saver = tf.train.import_meta_graph('./modeloEntrenado/model_3000_0.406236.meta')
new_saver.restore(session, tf.train.latest_checkpoint('./modeloEntrenado/'))
print(session.run(tf.get_collection(tf.GraphKeys.VARIABLES)))
...
def optimize(num_iterations):
    ...
    name = ('model_tuning' + str(conf.iterations) + '_' + str(val_loss)) 
    # Save model weights to disk
    save_path = saver.save(session, conf.model_path_tuning + name)
    tf.train.export_meta_graph(conf.model_path_tuning + name)
